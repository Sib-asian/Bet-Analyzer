...
# book che proviamo a usare per fare la media
PREF_BOOKS = [
    "pinnacle",
    "betfair",
    "bet365",
...
# ============================================================
#            FUNZIONI: THE ODDS API (per prendere match)
# ============================================================

def theodds_get_sports() -> List[dict]:
    """
    Prende tutti gli sport dall'API e filtra solo quelli di calcio (che iniziano con soccer-)
    """
    try:
        r = requests.get(
        ...
        data = r.json()
        # tengo solo soccer-*
        soccer = [s for s in data if str(s.get("key", "")).startswith("soccer-")]
        return soccer
    except Exception as e:
        print("Errore get_sports:", e)
        return []


def theodds_get_events(sport_key: str) -> List[dict]:
    """
    Per uno sport (es. soccer-italy-serie-a) prende tutti gli eventi con le quote.
    ATT: The Odds API dà gli eventi FUTURI, non storici.
    """
    try:
        r = requests.get(
            ...
            params={
                ...
                "regions": "eu,uk",     # dove prendiamo i book
...
def normalize_book_name(name: str) -> str:
    n = name.lower()
    # tanti book hanno suffissi tipo -eu, -uk ecc
    for pref in PREF_BOOKS:
        if pref in n:
            return pref
    return n


def theodds_parse_event(ev: dict) -> dict:
    """
    Da un evento di The Odds API tira fuori:
    - nome match
    - 1, X, 2 (media dei book buoni)
    - over 2.5, under 2.5 se ci sono
    """
    out = {
        "match_name": "",
        "odds_1": None,
    ...
    out["match_name"] = f"{home} vs {away}"

    bookmakers = ev.get("bookmakers", [])
    # liste per fare media
    h2h_home = []
    h2h_draw = []
    h2h_away = []
    over25 = []
    ...
    for bk in bookmakers:
        bk_name_norm = normalize_book_name(bk.get("title", ""))
        if bk_name_norm not in PREF_BOOKS:
            # se non è nella nostra lista, lo ignoriamo
            continue
        ...
        for mk in bk.get("markets", []):
            ...
            # h2h = 1X2 nel calcio
            if mkey == "h2h":
                # nel calcio dovrebbe esserci home, away, draw
                for o in outcomes:
            ...
            # totals = over/under
                ...
                # cerchiamo la linea a 2.5
                for o in outcomes:
                    point = o.get("point", None)
                    if point is None:
                        continue
                    # voglio proprio la linea 2.5
                    try:
                        if float(point) != 2.5:
                            continue
                    except:
                    ...
                    if "over" in name:
                        ...
                        under25.append(price)

    # faccio le medie
    def safe_avg(lst):
        return sum(lst) / len(lst) if lst else None

    out["odds_1"] = safe_avg(h2h_home)
    out["odds_x"] = safe_avg(h2h_draw)
    out["odds_2"] = safe_avg(h2h_away)
    out["odds_over25"] = safe_avg(over25)
    out["odds_under25"] = safe_avg(under25)

    return out


# ============================================================
#           API-FOOTBALL per aggiornare lo STORICO
# ============================================================

def api_football_get_fixtures_by_date(d: str) -> list:
    headers = {"x-apisports-key": API_FOOTBALL_KEY}
    try:
        r = requests.get(f"{API_FOOTBALL_BASE}/fixtures", headers=headers, params={"date": d}, timeout=8)
        r.raise_for_status()
...
st.set_page_config(page_title="Modello Scommesse – The Odds API PRO", layout="wide")
st.title("📊 Modello Scommesse – manuale + precompilazione da The Odds API")
...
if os.path.exists(ARCHIVE_FILE):
    ...
    st.write(f"Partite salvate: **{len(df_storico)}**")
    st.dataframe(df_storico.tail(30))
else:
    st.info("Nessuno storico ancora.")

# ------------------------------------------------------------
# 1. PRENDI PARTITA DALL'API
# ------------------------------------------------------------
st.subheader("🔎 Prendi una partita da The Odds API e riempi le quote")

# recupero sport (solo calcio)
soccer_sports = theodds_get_sports()
sport_keys = [s["key"] for s in soccer_sports]
sport_names = {s["key"]: s["title"] for s in soccer_sports}
...
if not soccer_sports:
    ...
    # sport scelto
    sel_sport = st.selectbox(
        "Seleziona campionato/lega (dalla tua API PRO)",
        options=sport_keys,
        format_func=lambda k: sport_names.get(k, k)
    )

    if st.button("Carica partite da questo campionato"):
        events = theodds_get_events(sel_sport)
        if not events:
            st.warning("Nessuna partita trovata per questo campionato in questo momento.")
        else:
            # salvo in session
            st.session_state["last_events"] = events
            st.success(f"Trovate {len(events)} partite per {sport_names.get(sel_sport, sel_sport)}.")

# se ho eventi caricati, faccio scegliere
if "last_events" in st.session_state and st.session_state["last_events"]:
    ev_map = {
        f"{e.get('id','')} – {e.get('home_team','')} vs {e.get('away_team','')}": e
        for e in st.session_state["last_events"]
    }
    sel_ev_label = st.selectbox("Seleziona partita da riempire", list(ev_map.keys()))
    sel_event = ev_map[sel_ev_label]
    parsed_ev = theodds_parse_event(sel_event)
    # metto in session le quote che ho trovato
    st.session_state["pre_match_name"] = parsed_ev["match_name"]
    st.session_state["pre_odds_1"] = parsed_ev["odds_1"]
    st.session_state["pre_odds_x"] = parsed_ev["odds_x"]
    st.session_state["pre_odds_2"] = parsed_ev["odds_2"]
    st.session_state["pre_over25"] = parsed_ev["odds_over25"]
    st.session_state["pre_under25"] = parsed_ev["odds_under25"]
    st.success("Quote prese dall'API e inserite nei campi sotto ✅")
...
match_name = st.text_input(
    ...
    value=st.session_state.get("pre_match_name", "")
)
...
with col_co1:
    ...
    odds_1 = st.number_input("Quota 1", value=float(st.session_state.get("pre_odds_1", 1.80) or 1.80), step=0.01)
with col_co2:
    total_co = st.number_input("Total corrente", value=2.5, step=0.25)
    odds_x = st.number_input("Quota X", value=float(st.session_state.get("pre_odds_x", 3.50) or 3.50), step=0.01)
with col_co3:
    odds_2 = st.number_input("Quota 2", value=float(st.session_state.get("pre_odds_2", 4.50) or 4.50), step=0.01)
    odds_btts = st.number_input("Quota GG (BTTS sì) – manuale", value=1.95, step=0.01)

st.subheader("3.b Quote Over/Under (da API se trovate)")
col_ou1, col_ou2 = st.columns(2)
with col_ou1:
    odds_over25 = st.number_input("Quota Over 2.5", value=float(st.session_state.get("pre_over25", 0.0) or 0.0), step=0.01)
with col_ou2:
    odds_under25 = st.number_input("Quota Under 2.5", value=float(st.session_state.get("pre_under25", 0.0) or 0.0), step=0.01)

st.subheader("4. xG avanzati (facoltativi)")
...
if not has_xg:
    st.info("Modalità: BASE (solo quote e linee).")
else:
    st.success("Modalità: AVANZATA (quote + linee + xG).")
...
if st.button("CALCOLA MODELLO"):
    ...
    if abs(delta_spread) < 0.01 and abs(delta_total) < 0.01:
        ...
        if abs(delta_spread) >= 0.01:
            st.write(f"Spread cambiato di {delta_spread:+.2f}")
        if abs(delta_total) >= 0.01:
            st.write(f"Total cambiato di {delta_total:+.2f}")
    ...
    if odds_over25 and odds_over25 > 0:
        ...
        diff_over = (ris_co["over_25"] - prob_over_book) * 100
        ev_over = ris_co["over_25"] * odds_over25 - 1
        ...
        rows_val.append({
            ...
            "Esito": "Over 2.5",
            "Prob modello %": round(ris_co["over_25"]*100, 2),
    ...
    if odds_under25 and odds_under25 > 0:
        prob_under_book = decimali_a_prob(odds_under25)
        diff_under = (ris_co["under_25"] - prob_under_book) * 100
        ev_under = ris_co["under_25"] * odds_under25 - 1
        ...
        rows_val.append({
            "Mercato": "Over/Under 2.5",
            "Esito": "Under 2.5",
            "Prob modello %": round(ris_co["under_25"]*100, 2),
    ...
    st.dataframe(pd.DataFrame(rows_val))
...
# ------------------------------------------------------------
st.subheader("🗑️ Cancella una analisi dallo storico")
if os.path.exists(ARCHIVE_FILE):
    ...
    if not df_del.empty:
        ...
        if st.button("Elimina selezionata"):
            ...
            df_new.to_csv(ARCHIVE_FILE, index=False)
            st.success("Analisi eliminata. Ricarica la pagina.")
    else:
...
# ------------------------------------------------------------
# SYNC RISULTATI (solo per lo storico) usando API-Football
# ------------------------------------------------------------
st.subheader("🔄 Aggiorna risultati reali nello storico (API-Football)")
if st.button("Aggiorna ultimi 3 giorni"):
    if not os.path.exists(ARCHIVE_FILE):
        ...
        for d in giorni_da_controllare:
            fixtures = api_football_get_fixtures_by_date(d)
            for f in fixtures:
